...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		//$this->language = new Language($order_info['language_code']);
		//$this->language->load($order_info['language_code']);
		$this->language->load('mail/order_add');

		// HTML Mail
		$data['title'] = sprintf($this->language->get('text_subject'), $this->config->get('config_name'), $order_info['order_id']);
		...
		$data['logo'] = $this->config->get('store_url') . 'image/' . $this->config->get('config_logo');
		$data['config_name'] = $this->config->get('config_name');
		$data['store_url'] = $this->config->get('store_url');
		$data['customer_id'] = $order_info['customer_id'];
		$data['link'] = $this->config->get('store_url') . 'index.php?route=account/order/info&order_id=' . $order_info['order_id'];

		if ($download_status) {
			$data['download'] = $this->config->get('store_url') . 'index.php?route=account/download';
		} else {
			$data['download'] = '';
		}
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			foreach ($order_products as $order_product) {
				...
				$data['products'][] = array(
					...
					'quantity' => $order_product['quantity'],
					'option'   => $option_data,
					'total'    => html_entity_decode($this->currency->format($order_product['total'], $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8')
				);
			}
