...
class ControllerStartupSeoUrl extends Controller {
	private $cache_data = null; //кеширование
	
	/*
		Конструктор контроллера SeoUrl.
		Кеширует данные из таблицы seo_url.
	*/
	public function __construct($registry) { 
		parent::__construct($registry);
		$this->cache_data = $this->cache->get('seo_url');
		if (!$this->cache_data) {
			$query = $this->db->query("SELECT LOWER(`keyword`) as 'keyword', `query` FROM " . DB_PREFIX . "seo_url ORDER BY seo_url_id");
			...
			foreach($data as $key => $value) {
				switch($key) {
					case 'useful_id':
						...
						if ($query->num_rows) {
							$seo_url .= '/useful/' . rawurlencode($query->row['keyword']);
							unset($data[$key]);
							$postfix = 1;
						}
						break;
