...
class ModelCatalogAttributeGroup extends Model {
	public function addAttributeGroup($data) {
		...
		if ($data['attribute_group_description']) {
			$value = $data['attribute_group_description'];
			$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_group_description SET attribute_group_id = '" . (int)$attribute_group_id . "', name = '" . $this->db->escape($value['name']) . "'");
		}
	...
	public function getAttributeGroups($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "attribute_group ag LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id)";
	...
	public function getAttributeGroupDescription($attribute_group_id) {
		...
		if ($query->num_rows){
			$attribute_group_data =array('name' => $query->row['name']);	
		}
		

		return $attribute_group_data->row;
	}
